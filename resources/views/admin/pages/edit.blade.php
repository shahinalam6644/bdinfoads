   ...
   <div class="row">
        <div class="col-lg-12">
            ...
            <div class="text-center">
                <h1 class="h4 text-gray-900 mb-4">Edit a Page!</h1>
            </div>
            <form class="user" action="{{ url('/dashboard/pages/') }}/{{ $pages->id }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @method('PATCH')
                ...
                <textarea id="inp_editor1" name="content" value='{{ $pages->content }}'>{{ $pages->content }}
                </textarea>  
                ...
                <div class="form-group row">
                    <div class="col-sm-10 mb-3 mb-sm-0">
                        ...
                        <option value="0" {{ $pages->status == 0 ? 'selected' : '' }}  >Unpublish</option>
                        <option value="1" {{ $pages->status == 1 ? 'selected' : '' }}  >Publish</option>
                        </select>
                    </div> 
                    <div class="col-sm-2 mb-3 mb-sm-0">
                        <input type="hidden" id="type" name='thumbnail_path' placeholder="Image Url" class="form-control" value="{{ $pages->thumbnail_path }}">
                        <img id="myImg" src="{{ $pages->thumbnail_path }}" width="100%" height="auto" data-toggle="modal" data-target="#exampleModalCenter" class="border border-info">
                    </div> 
                </div>
